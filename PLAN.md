...
- **L3 实测修复（2026-08-15 晚，用户两问题 + 自动化实测发现的尺寸 bug）**：
  ...
  - **L3.1 增补（2026-08-16，用户反馈）**：①**恢复默认外观**按钮（外观区颜色区块底部）：行高/分组展开/组头封面/字体/字号/强调色/透明度/列顺序/列宽全部复位；②**风格预设**（外观区顶部）：6 套一键组合（默认清爽/海洋蓝/薄荷绿/落日橙/紫罗兰/紧凑大字），每套 = 强调色（染色关闭时生效）+ 行高 + 字号 + 选中/悬停透明度；应用后设置页控件同步刷新（ReloadUiProperties 重读 config + OnPropertyChanged）。UIA 实测：预设「海洋蓝」→ accent=#0288D1/row=72/scale=1/sel=0.14/hover=0.08 ✓；恢复默认 → 8 列默认序/columnWidths 空/accent 空/72/1.0/true/true ✓。harness 回归 56 项零失败。
