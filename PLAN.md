...
- step7 快捷键自定义改绑 ✔（2026-08-15 用户目验反馈提前交付）：ShortcutMap（Player.Core/Hotkeys，纯函数可 harness：默认表 + 配置覆盖 UiConfig.ShortcutBindings，非法组合/组合冲突/字母键无修饰一律回退默认；组合串规范 Ctrl+Shift+Alt+K）；快捷键页每行「改绑」→ 捕获按键（Esc 取消；字母键须带修饰；占用/重复明确提示；改绑即落盘并立即生效）；全局热键组合同样可改（UiConfig.GlobalHotkeyCombos，须带修饰键、防三键互相重复，改绑后强制重注册，占用提示沿用逐条弹窗；注册失败日志带 Win32 错误码）。harness shortcuts 扩至 32 项全过；UIA 驱动端到端验证：F5→F6 改绑落盘并即时反映在列表、Esc 取消不落盘。注：注册成功/触发/失败均留日志，用户侧"全局热键不生效"可用日志区分 未启用/注册失败/未触发。
- 用户目验修复（2026-08-15）：桌面歌词锁定态解锁柄初始隐藏（此前 Opacity 未归零，鼠标不在歌词区域也显示锁）——锁定/重新锁定时 Opacity=0 + _handleHot 复位，悬停才淡入，截图验证锁定态无锁柄；另附：全局热键注册成功日志（排查不生效用）。
