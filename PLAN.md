...
> **（2026-08-14 追记）P3.1 的 ①② 经八次提交（315c63b~c19e021）仍未修复歌词显示，用户决定推倒重做；P3.2 未启动。两者一并由下面的 UI-R 阶段取代。P3.1 的 ③内嵌歌词 ④额度迁移已落地、保留。结论存档：基于 ItemsControl/DataTemplate/ScrollViewer 的歌词渲染路径在本项目判死刑，禁止再修，重做必须走自绘路线。**

### UI-R 歌词页重做与界面重构（取代 P3.1①②与 P3.2，分五步，每步提交+发布+用户目验后才进下一步）

目标观感 = 用户提供的 foobar 截图（黄色专辑那张）：窄侧栏、专辑分组列表、右侧信息栏、**整套界面配色随封面主色染色**（浅色 tint 背景 + 深色文字 + 高饱和强调色）。

**UI-R0 歌词渲染重做（最优先）**
- 删除现有歌词覆盖层的全部展示实现（XAML、code-behind、VM 展示部分），不留死代码；LyricsService/缓存/匹配/来源选择等数据层保留。
- 新建**自绘歌词控件**：单个自定义 FrameworkElement，OnRender + FormattedText 绘制可见行，自行管理滚动偏移（当前行居中为目标位置 + 缓动动画）。禁止使用 ItemsControl/ListBox/DataTemplate/ScrollViewer 组合。
- 交互：点击行=跳转；滚轮=临时自由浏览，静置 3 秒回到跟随；当前行加粗+强调色，相邻行淡出。
- 形态改变：歌词不再是浮动覆盖层。位置一=右侧信息栏内嵌（默认）；位置二=主区正常导航的全页视图（有返回按钮，Esc 也可退）。重新获取/重新匹配/来源选择收进歌词区域右键菜单（此交互已验证可用，保留）。
- 验收：文本正确显示（类型名 bug 消失）、居中平滑滚动、点击跳转、滚轮浏览自动回位、两个位置都正常。

**UI-R0 已知问题（2026-08-14 用户留存待办，随歌词相关修改一并处理）**
- 问题：右侧歌词栏很窄时，歌词行显示不全，以省略号截断部分歌词（当前 LyricCanvas/展示层对窄面板的处理不足）。
- 解决方案方向（用户给定）：面板过窄无法完整显示歌词时，**减少显示的歌词句数**（而非截断文字），保证最窄时最少能完整显示**一句当前播放的歌词**。
- 同时留意日志中 LyricCanvas.OnRender 的 ArgumentOutOfRangeException（切歌/歌词刷新偶发，UI 线程未处理异常被捕获记录），一并纳入歌词相关修复。

**UI-R1 布局骨架**：左侧栏收窄（约 200px，可拖宽）——顶部搜索/过滤框、媒体库、媒体库视图、歌单列表（各带计数）；主列表区；右侧信息栏（可折叠，宽约 280px）；播放条重排——左：播放模式图标组+设置入口，中：上一曲/播放暂停（圆形强调色）/下一曲/停止，右：歌词开关/随机/音量条；窗口标题栏显示「标题 - 艺术家 | 格式 | 位深 | 码率 | 采样率」。

**UI-R1.5 细节打磨（2026-08-14 用户目验 R0/R1 后追加）**：R0 歌词渲染验收通过、R1 骨架成立；方向修正——**不照抄 foobar，简洁优先**。打磨范围：清除界面上的开发者信息（扫描统计常驻文本、歌词匹配来源与 ID）、删除操作提示语、侧栏底部无名图标组重构、播放模式多按钮收敛为单按钮循环切换、输出信息并入播放条徽章并挂设备快捷切换菜单、标题栏码率格式化、选中行改淡色填充、补时长列、歌单空态改可点击新建。完成并经用户目验后才进 R2；R2 从简（组头+封面列即可）。

**UI-R1.5 目验结论（2026-08-14）：全部通过**。经历十二轮反馈迭代（导入入设置页、播放条两行布局、进度条贯穿+按钮对齐、大行列表 110px+封面 84、文字对齐、模式按钮左键循环/右键菜单、输出/歌词图标重绘、音量 10 格方块+dB 浮层、窗口与状态记忆、UIA 逐像素校验等），最终形态：播放条=上行完整进度条（时间文字两端=最外侧按钮外缘）+下行左按钮组/居中控制键/右按钮组；列表=封面 84 正方形+专辑字在上、歌名/歌手/时长/格式/采样率/位深单行对齐；音量=10 格方块（已到浅灰/未到深灰、拖动显示右侧 dB 文字）；设置旁 3 个占位按钮为用户要求预留。


**=========== R1 目标与成果 / R1.5 用户额外要求与成果（R2 重新规划基线，2026-08-14 定稿） ===========**

**一、R1 原始目标（PLAN 原定义）**
左侧栏收窄（约 200px，可拖宽）——顶部搜索/过滤框、媒体库、媒体库视图、歌单列表（各带计数）；主列表区；右侧信息栏（可折叠，宽约 280px）；播放条重排——左：播放模式图标组+设置入口，中：上一曲/播放暂停（圆形强调色）/下一曲/停止，右：歌词开关/随机/音量条；窗口标题栏显示「标题 - 艺术家 | 格式 | 位深 | 码率 | 采样率」。

**二、R1 成果（已交付）**
- 提交 65f9295：侧栏 200px 可拖宽、右侧栏 280px 可折叠（ToggleSidePane）、播放条重排（圆形强调色播放键）、标题栏技术信息（含码率格式化：<1000 显示 "N kbps"，≥1000 显示 "X.X Mbps"）。
- 配套 R0 系列（89d428b/0c70e9e/59eed06）：自绘歌词控件 LyricCanvas（FormattedText 缓存、缓动滚动、点击跳转、滚轮浏览）、歌词来源/模式/偏移收进右键菜单、右侧栏大封面。

**三、R1.5 用户额外要求与成果（十二轮反馈，全部交付并经 UIA 逐像素实测）**
1. 视觉打磨：删操作提示语（双击/右键提示、歌词区提示）；选中行改强调色 12% 透明淡填充（无边框）；补右对齐时长列；封面统一 2px 圆角与占位图；歌词来源移入右键菜单（"来源：网易云 · 缓存"，去数字 ID）；标题与歌词间距；扫描只留细进度条+状态 4 秒自消；空歌单改可点击"＋ 新建歌单"；激活态改强调色 tint。
2. 布局重构：导入 m3u8 移入设置页；设置按钮缩为图标移至播放条左侧；播放条改两行——上行完整进度条（时间文字 0:00/4:15 两端=最外侧按钮外缘，整体缩短 10% 居中），下行左按钮组（设置+3 个预留占位）/居中控制键（文件夹箭头/上一曲/播放/下一曲/停止，播放按钮居中于进度条）/右按钮组（歌词/模式/定位/输出+喇叭+音量）；播放模式键左键循环切换、右键弹出四项菜单（图标随模式、单选）；歌词面板图标 AlignLeft24、输出图标 PlugConnected24；临时播放文件夹按钮（默认打开媒体库根目录、不入库直接播）。
3. 列表大行化：行高 110px（默认窗口约 6 首）；封面 84×84 正方形、专辑名小字置于封面上方；歌名/歌手/时长/格式/采样率/位深单行对齐（标题顶端=封面顶端，歌手与信息列同字号 10.5）；文字下方留空；全局细滚动条（8px 圆角）；默认窗口 1400×900 且记忆窗口尺寸。
4. 音量控件：10 格等距实心方块（已到达=浅灰 #8B8B8B、未到达=深灰近黑 #3A3A3A；5×4px 直角、间距 4px；点击/拖动设置）；拖动时在音量格右侧短暂显示 dB 文字（0dB=100%、-100dB=静音，松手 1 秒消失，鼠标捕获保证控件外松手也隐藏）。
5. 状态记忆：启动恢复上次停留页面（歌单/文件夹）与上次播放曲目（右侧栏显示信息与歌词，不自动播放）。
6. 修复（UIA 实测定位）：GridView 单元格按内容高度居中导致歌名比封面低 33px → 单元格内容固定 110px 容器对齐；播放按钮偏左 67px → 传输组横跨整行居中；模式图标不跟随/菜单可多选 → 补 IsXxxMode 变更通知 + SetPlayMode 强制重同步。

**四、R2 规划基线（事实清单，避免再出现信息不对称）**
- 播放条最终布局如上述第二条；设置旁 3 个占位按钮为**用户要求预留**（非残留）；音量点阵为**最终形态**（非待改项）。
- 列表当前为未分组的平铺大行（封面 84 + 行上专辑小字），是 R2 分组改造的起点。
- 无遗留 R1.6 待办；R2 从简原则不变（组头+封面列即可），具体方案待与用户审验后定稿。

**UI-R1.5 目验反馈（2026-08-14 第二轮）**：导入 m3u8 移入设置页（媒体库组）；设置按钮缩小移到播放条左侧曲目信息下方（原播放模式位）；播放模式循环键与定位键移到音量右侧；输出按钮缩成图标，点击展开全部后端设备列表（ASIO/WASAPI/系统输出 平铺、当前项打勾）；曲目列表封面改 26×26 正方形（原 38 被 28px 行高压扁）；列表横向滚动条删除，默认窗口 1200→1480 保证七列全显；播放条左下封面（原歌词页入口）删除，只留标题/艺术家。

**UI-R1.5 目验反馈（2026-08-14 第三轮）**：音量右侧的按钮全部移到音量条左侧（右组=输出/歌词/模式/定位 + 音量）；上一曲/播放/下一曲/停止移到进度条下方；删除播放条左下角歌名歌手文字；搜索框占位符改「搜索当前列表」；设置按钮与右侧按钮行对齐；底栏按钮整体加大（模式键 30×28、传输键 36×36、图标 16/20）；设置按钮旁预留 3 个占位按钮；曲目列表改大行 84px（一首约三行面积），封面 64×64 正方形+专辑名置于封面上方，删专辑列、歌手并入标题下，文字不放大、下方留空；全局细滚动条（8px 圆角）；默认窗口 1400×900；窗口尺寸记忆；启动恢复上次停留页面（歌单/文件夹）与上次播放曲目（右侧栏显示信息与歌词，不自动播放）。

**UI-R1.5 目验反馈（2026-08-14 第四轮）**：底栏两侧按钮向内靠（边距 16→10）；进度条拉长贯穿两组按钮之间、两端恰与按钮组对齐，时间文字移到按钮组外侧；音量百分比删除；曲目行 84→110px（默认窗口约 6 首），封面 64→84 正方形、专辑字放大（12.5 更亮）、标题/歌手恢复分列且字号略缩（12/10.5）；右组顺序改为 歌词→模式→定位→输出；播放模式改点击弹出四项选择菜单（顺序/列表循环/单曲循环/随机，勾选当前），图标随模式变化但不再强调色高亮，切换不再刷状态栏文字；歌词面板图标换 AlignLeft24（原 DocumentText24 像保存）；输出图标换 PlugConnected24（与音量喇叭区分）；控制键缩至 32/40 并居中于进度条，上一曲左侧新增「临时播放文件夹」按钮（默认打开媒体库根目录，不入库直接播）。

**UI-R1.5 目验反馈（2026-08-14 第五轮）**：播放条改两行——第一行完整进度条（时间文字+滑条，总长与第二行按钮两端对齐），第二行左按钮组/控制键/右按钮组；列表文字对齐——标题顶端与封面顶端对齐（y≈24），歌手与时长/格式/采样率/位深同行同字号（10.5，y≈42），信息列由 DisplayMemberBinding 改为显式 CellTemplate 统一样式。

**UI-R2 专辑分组列表（2026-08-14 与用户定稿：封面列式）**：分组键=专辑+专辑艺术家，组内按碟号/曲号排序；组头一条细行（专辑名 | 艺术家 ———— 年份右对齐）；**封面从曲目行移出，组内左侧独占一列，一组一张 84px、与组首对齐**（现在每行封面+封面上方专辑小字的形态废除）；曲目行瘦身到约 56px 两行结构——上行标题（12）、下行歌手（10.5），时长/格式/采样率/位深信息列保留、垂直居中；无专辑标签的散曲归入「单曲 | 艺术家」组；当前播放行淡色填充+行首 ▶；保持虚拟化流畅；列表右上小图标切换 平铺/分组，选择持久化。

**UI-R3 封面取色主题引擎（2026-08-14 与用户定稿：整体染色）**：从当前曲目封面提取主色与配色（缩至 ~32×32 后直方图/聚类）；**整套界面随封面染色**——窗口背景浅 tint、面板/侧栏/播放条同系派生色、强调色高饱和、文字深色（参考用户 foobar 截图的黄色主题效果）；派生算法必须带对比度保底（文字/背景对比不足时自动加深或回退），过暗/过灰封面回退中性浅灰配色；切歌 300ms 平滑过渡；设置页两挡：**跟随封面（默认）/ 固定深色**（保留逃生口）；取色与派生纯函数进 harness（含对比度断言）。

**UI-R3 目验结论（2026-08-14）：通过（用户「无可挑剔」）**。经历五轮反馈：①浅色模式设置页难读 → WPF-UI 控件主题随底色联动（Light/Dark）；②设置页分页（主题/输出/媒体库/在线四页）、主题改为**底色（深/浅）× 染色（开/关）四模式自由组合**（浅色+染色为默认）；③统一染色——AccentFill 系列资源随染色覆盖（按钮/选中态跟随封面强调色），深色染色加强（黑混合 86%→78%/70%，保留明显色相）；④顶栏（WPF-UI TitleBar Background 绑定 SurfaceStrongBrush）与底栏（SurfaceStrong 0.82→0.72）补齐染色；⑤浅色染色增强（背景白混合 78%、面板 86%、顶底 83%，五块区域同档一致）。最终形态：底色×染色四模式、全页面统一、harness 17 项断言（含深浅两套对比度保底）。

**UI-R4 右侧信息栏完善（2026-08-14 与用户定稿：不做星级）**：大封面、标题、艺术家 | 专辑、制作信息（作词/作曲/编曲——来源为标签或 LRC 头部元数据，有才显示，放歌词流之外的独立区块）、歌词联动预览（复用 UI-R0 控件）；折叠状态持久化。星级评分整项取消，不做列表等级列。

**UI-R5 歌词排版精修（2026-08-14 立项，对照 foobar 歌词栏逐条修）**：用户以 foobar 歌词栏为标杆给出六条差距，LyricCanvas 布局逻辑按此重写：

1. **元数据彻底出流**：作词/作曲/编曲/OP/ED 等头部行（含 0:00 时间戳形态、全角/半角冒号、前后空格变体）从歌词时间流中剥离，并入右栏"制作信息"区块（与标签来源合并去重）；歌词流内不再出现、永不参与当前行判定——开播第一句真实歌词出现前，无当前行高亮。
2. **全部水平居中**（含折行后的续行）。
3. **成对单元布局**：原文+翻译=一个布局单元，单元内间距 2–4px（翻译字号 ≈ 原文的 0.85、颜色更淡），单元间距统一 14–18px；无翻译的单元不保留空位，动态高度。
4. **自动折行禁止截断**：按栏宽折行——CJK 逐字符、拉丁按词；折行续行同样居中；任何情况下不出现省略号。
5. **整对高亮**：当前单元原文加粗+强调色，其翻译同强调色略淡；非当前单元统一次级色。
6. **动态重排与居中滚动**：单元高度可变，滚动目标=当前单元几何中心（缓动保留）；栏宽变化（拖动分隔条/折叠展开/窗口缩放）即时重排重折行，布局缓存按宽度失效。

验收：对照 foobar 截图逐条目验；纯文本无时间轴歌词=整篇静态居中；harness 补布局纯函数断言（折行、单元高度、居中偏移、元数据识别样式至少 6 种变体）。

### L1 歌词体验三连（UI-R5 精修 + 大歌词页 + 桌面歌词，2026-08-14 立项）

- **第一步 UI-R5**：六条精修见上节。
- **大歌词页**：播放条左下预留按钮①启用为入口（再次点击/Esc 返回）。主区整页：放大模糊的封面做背景（叠加取色渐变遮罩保证可读性），中央大字号歌词（复用 R5 排版引擎与元数据规则，字号约平时 1.6–2 倍），底部细进度条+基本控制键。进出 200ms 级淡入淡出，滚动缓动沿用。目标是"好看"：这一页允许比主界面更有表现力。
- **桌面歌词**：置顶无边框透明条（默认屏幕下方，可拖动），当前句原文+翻译双行；描边/阴影保证任意壁纸可读，颜色联动取色主题；锁定=鼠标穿透（缩成小柄可解锁），解锁=可拖动调宽；字体/字号/单双行进设置「歌词」组；开关=预留按钮②，托盘菜单同步提供；无时间轴歌词显示曲名。
- 按钮③继续留白（候选：播放队列面板/迷你模式，用户定）。

**L1 审查与目验记录（2026-08-15）**：子代理审查判 4 个 BLOCKER——B1 桌面歌词锁定状态取反（锁定/解锁语义颠倒）；B2 解锁时误清 WS_EX_LAYERED（会破坏逐像素透明）；B3 大歌词页进度条未挂 Begin/EndSeek 接管（弹回类缺陷第三次复发）；B4 ShutdownMode 未设导致关主窗残留进程。修复方案已批准。**新工程铁律：凡绑定播放位置的 Slider，一律使用统一的 SeekSlider 接管组件（把主滑条那套 4×AddHandler + 乐观更新 + 静默窗口封装成可复用 attached behavior），禁止散装复制——此类缺陷已三次复发。**

**L1.1 打磨（用户目验追加四组）**：
1. **音量格颜色语义修正**：R1.5 写死的 #8B8B8B/#3A3A3A 在 R3 浅色主题下明暗倒挂。改为主题语义色——已到达格=前景强调（深/醒目），未到达格=弱化色（浅），两挡主题下都成立，不再写死灰度。
2. **歌词字体设置**：设置「歌词」组加字体下拉（枚举系统已装字体，置顶常见中日文友好项：微软雅黑/思源黑体/Noto Sans CJK/等线等）+ 字重（常规/中等/加粗），作用于右栏、大歌词页、桌面歌词三处（桌面歌词字号维持独立）。
3. **桌面歌词个性化补全**：背景卡片可隐藏（纯文字模式，靠描边/阴影保可读）或调透明度；右键菜单：锁定/解锁、显示翻译、隐藏背景、字号±、字体、关闭桌面歌词；悬停显示迷你工具条（含锁定按钮，解决"解锁后无途径再锁定"）；文字颜色：跟随取色主题 / 自定义纯色。
4. **大歌词页收尾**：修复"再点按钮①退出"失效（现仅 Esc 有效），并增加两种退出：点击空白区域、右上角关闭按钮；底部控制条默认收敛为一条细进度线，鼠标活动时淡入完整控制（含可 seek 滑条），3 秒无操作淡出——消除"突兀"；删除左下角遗留的空框控件。

> **大歌词页终版形态（2026-08-15 用户目验定稿，取代上述 ④ 的中间方案）**：不要 ×、不要控制条/进度条/时间文字；退出=Esc 或**双击任意位置**；**点击歌词左半=上一曲、右半=下一曲**（LyricCanvas 增 ClicksEnabled=false 交还宿主处理）；页面只剩 模糊封面背景+大号歌词，极致沉浸。

**L1.1 实施与目验记录（2026-08-15，UIA 截图 + 视觉模型逐状态复核）**：
- **B5（目验发现，修复）**：锁定=鼠标穿透用 WS_EX_TRANSPARENT 会让整窗（含小柄）收不到鼠标，锁定后永远无法解锁（实测点击小柄配置不翻转）。改为 WM_NCHITTEST 逐点判定：小柄 26×26 区域返回 HTCLIENT（可点），其余返回 HTTRANSPARENT（穿透）。B1 修复后此缺陷才暴露，二者互为依存。
- **控制条隐形命中修复**：控制条淡出后仍可命中，底部点击会误触隐藏滑条/按钮（实测"点击空白退出"在底部条带失效的根因）。修复：淡出同时 IsHitTestVisible=false，淡入恢复。
- **W1 扩展**：未开始播放（CurrentIndex<0）时桌面歌词同样显示曲名，避免空条（实测停止态桌面歌词条为空白）。
- **空框确认**：左下角"空框"=播放条预留按钮③（占位按钮，0.22 透明）。用户目验判定为遗留，**已删除**；L1 立项的"按钮③继续留白"相应撤销，后续候选（播放队列面板/迷你模式）直接进 backlog。
- **音量格目验**：浅色主题下已到达=前景深色、未到达=浅灰（语义色 + 22% 透明度），正常。
- **大歌词页目验**：模糊封面 + 大号双语歌词 + 当前行橙色高亮 + 细进度线 + 右上 × 关闭 + 点击空白退出 + 控制条 3 秒收敛，全部正常。

**L1.1 第二轮目验（用户实测，6 项修复，2026-08-15）**：
1. **背景卡片隐藏后歌词文字也消失**（文字在背景 Border 内）→ 文字层移出背景卡片为独立兄弟层，隐藏背景不丢文字。
2. **锁定只锁位置不透明** → 锁定（鼠标穿透）态强制纯文字模式（背景透明、描边阴影加强），解锁态才按「显示背景卡片」设置显示。
3. **歌词条文字不居中** → 行高 Auto 打包在顶部；文字层改垂直居中（Grid VerticalAlignment=Center）。
4. **大歌词页出现「大歌词页」小框** → 按钮 ToolTip 弹出层（独立置顶窗口）在覆盖层打开后仍悬浮；打开覆盖层/桌面歌词前强制关闭对应 ToolTip（ToolTip 改为显式 x:Name 元素）。
5. **大歌词页 × 与窗口 × 重叠会关掉软件** → 覆盖层 × 落在窗口非客户区标题栏按钮位置；下移至标题栏之下（Margin top 62）。
6. **大歌词页控制条与页面风格不统一** → 为大歌词页重设控制条 UI：无底色（借底部渐变融入）、3px 细滑条 + 14px 圆钮（BigLyricsSeekStyle）、40px 幽灵按钮（BigLyricsGhostButtonStyle）、46px 圆形播放键；自动收敛行为不变。

**L1 验收通过（2026-08-15，用户实机目验「完美」）**：大歌词页交互终版（拖动滚动/滚轮滑行/点行跳转/分区切歌/双击与 Esc 退出/空格暂停/无焦点框）+ 桌面歌词全部功能 + 音量格语义色 + 歌词字体设置，经八轮目验迭代定稿。全 harness 184 项通过（seamless 26 / lyrics 113 / grouping 13 / theme 22 / library 10）。L1 关闭，进入 L2。

**L1.1 第三轮目验（用户实测，2026-08-15）**：
1. **背景隐藏后右侧出现"小蓝条"** → 即解锁态调宽柄悬浮在透明背景上；背景隐藏时调宽柄一并隐藏（调宽恢复背景后可再拖）。
2. **设置页文字颜色切换不生效** → 钩子只调 ApplySettings 未应用前景色；ApplySettings 末尾补 ApplyTextColor()。
3. **右键菜单子菜单难用**（子项偏移、鼠标斜出即消失、跨项误切）→ 弃用系统 ContextMenu 子菜单，改为**自定义悬浮面板**（右键弹出 Popup：平铺按钮行 + 字号± + 透明度四档 + 文字颜色色板 + 字体设置入口 + 关闭），悬停高亮、点击即生效，无子菜单走廊问题。
4. **「矩形小框」仍在 + × 位置奇怪** → 小框为覆盖层元素 ToolTip 悬浮气泡（× 的「关闭大歌词页（Esc）」）；去掉 × 的 ToolTip，× 改幽灵样式并重摆位（Margin 0,60,26,0）。
5. **UI 布局参考**（用户提供另一播放器截图：大封面背景+现代控制条+设置侧栏）→ 记入 backlog，L2 起评估（本轮不扩范围）。

### L2 本地基础补全（快捷键 / SMTC / 托盘 / 设置页，2026-08-14 立项）

- **应用内快捷键**：Space 播放暂停、←/→ seek ±5s、Ctrl+←/→ 上下曲、Ctrl+F 聚焦搜索、Enter 播放选中、Delete 歌单内移除、Ctrl+L 定位当前曲、F5 重扫；设置「快捷键」页列出并可改绑（2026-08-15 用户目验要求提前交付，原 backlog 项撤销，见 L2 实施记录 step7）。
- **多媒体键与 SMTC**：接入 Windows SMTC——键盘媒体键控制播放，锁屏/音量浮窗显示封面、标题、艺术家，状态实时同步。
- **全局热键**：默认关闭可选开启（预设 Ctrl+Alt+P / ←/→），注册冲突时明确提示。
- **托盘**：可选"关闭最小化到托盘"；托盘菜单：播放控制/桌面歌词开关/显示主窗/退出；双击还原。
...
### L3 体验打磨与个性化（2026-08-15 立项，P4 收官后的用户目验清单；顺序 L3.0→L3.1→L3.2）

**L3.0 缺陷与一致性小修**
1. **焦点框第三次出没**：艺术家视图点击后按方向键出现系统焦点虚线框。这次全局根治——App 级统一 FocusVisualStyle 策略（列表/按钮/胶囊分页/侧栏全套置空或替换为主题样式），键盘导航功能保留；子代理全仓扫一遍可聚焦控件的 FocusVisualStyle 覆盖情况，逐个列表核销。
2. **下载页控件主题不符**：下载确认框与下载管理页的 CheckBox/ComboBox/按钮/输入框全部接主题刷子（DynamicResource），深浅两挡+染色开关四种组合下都协调；顺带全仓扫一遍仍在用默认样式的对话框控件。
3. **设置页排版重构**：8 个胶囊分页已拥挤。改为**左侧竖排窄导航**（参考 foobar 参数选项的树形分区：外观/播放/资料库/在线/系统 五大区，区内小节），右侧内容区统一栅格间距、内容最大宽度约 560px 防超宽拉伸；现有全部选项按新分区归位。

**L3.1 个性化选项扩充（参考 foobar/foobox，只摘实用项）**
1. 列表：行高三挡（紧凑 56 / 标准 72 / 舒适 110，作用于平铺与分组）；分组默认 折叠/展开 + 记住每组状态；分组标题显示封面 开关。
2. 列自定义：显示/隐藏 + 顺序调整（标题/歌手/专辑/时长/格式/采样率/位深/码率），列宽持久化。
3. 界面字体：全局 UI 字体下拉 + 字号缩放（90%–125%），独立于歌词字体。
4. 颜色：染色关闭时提供自定义强调色色板；选中/高亮透明度微调。
全部持久化；harness 补配置往返断言；选项归位到 L3.0-3 的新设置分区。

**L3.2 迷你悬浮窗（启用预留按钮④，最后一个空位）**
- 形态：置顶小窗（约 340×96 横向卡片，圆角、主题染色跟随）；左封面、中标题/艺术家（超长跑马灯）、底部细进度线；悬停淡入控件：上一曲/播放暂停/下一曲/回主窗；双击=还原主窗；可拖动+位置记忆；按钮④再按或 Esc 关闭。开启迷你窗时主窗隐藏。
- 可选频谱（设置可关）：**技术红线——绝不能对解码流/混音流直接 ChannelGetData 消费数据（会抢 ASIO 的数据）**，正确做法是在 mixer 上挂 DSP tap 把样本复制进环形缓冲，UI 侧对缓冲做 FFT 渲染小频谱柱，约 30fps，CPU 开销可忽略；先做探针验证再进主工程。
...
- 遗留待用户目验：迷你窗外观细节（封面/悬停控件/频谱柱效果）、行高/列设置/折叠交互、设置页新分区整体观感、深浅两挡+染色四组合下的新控件配色。→ **已全部验收（2026-08-16，用户实机 5 项全过：默认输出频谱无抽动、ASIO 32/128/256 三档无爆音断音、迷你窗拖动跟手+四边四角等比缩放+双击恢复 340×104、多封面下迷你窗颜色固定仅随深浅、主窗/迷你窗/桌面歌词/托盘组合显隐退出无 Player.exe 残留）**。
- **L3.2 用户实装（2026-08-16，提交 ddae980，子代理审查通过后验收归档）**：完整迷你悬浮窗——340×104 默认，80%-200% 整体等比缩放（WM_SIZING 约束，四边四角，双击非交互区恢复默认）；原生移动循环拖动（WM_NCLBUTTONDOWN+HTCAPTION），位置/显示器/比例持久化（显示器设备名 Base64 + 工作区 DIP 偏移，失效显示器纠正回主屏，DPI 变化二次 clamp）；移动期间冻结频谱/跑马灯/悬停布局；64×64 封面、可拖动进度条、悬停控件不挤歌曲信息；迷你窗染色整体删除（固定深浅调色板，仅随 DarkBase，主窗染色不受影响）。新增 AppSurfaceCoordinator 统一主窗/迷你窗/托盘/桌面歌词显隐与退出（退出幂等、SessionEnding 放行、托盘建立失败不隐藏主窗成残留进程）；UI 侧栏与列表间缝隙收敛 1px（保留透明拖拽命中区）。音频管线重构：mixer DSP 回调只复制 PCM 到预分配 SPSC ring（无分配/无锁/无 FFT/无日志，epoch+generation 防旧回调 ABA）；后台分析线程 Hann 窗 + 48/96/192kHz→2048/4096/8192 点 FFT + 对数 16 柱 + 双缓冲 Seqlock 发布；频谱租约引用计数（最后消费者释放真卸 DSP，mixer 重建只恢复当前注册）；DirectSound UpdatePeriod=20ms + 180ms 宽限期防分批供样误衰减。ChannelGetData 全项目仅 WASAPI sink 一处，频谱路径零调用。harness 扩至 283 项（downloads 23 + dsp 44 + netfail 4）。Release 0 错误；283 通过 0 失败 1 跳过（仅 ASIO 实机听测）。子代理审查：红线/SPSC/租约/协调器/缩放/位置持久化全部通过，仅 3 条低优先级建议（dropped 计数非原子、Stop drain 自旋、读者自旋理论边界）不阻塞。**L3 至此全部完成并归档**。
- **L3 实测修复（2026-08-15 晚，用户两问题 + 自动化实测发现的尺寸 bug）**：
  - ①列表错乱/平铺空白根因：L3.1 列自定义实现时 VM 只写了私有 ColWidth()/ColIndex() 方法，**没暴露 ColWidthTitle/ColIndexTitle 等 16 个公开绑定属性**——列头与行模板的列宽/列索引绑定全部失败（宽 0、Grid.Column=0），平铺时封面列宽 0 → 整列表不可见，分组时元素堆进 36px 封面列 → 视觉错乱。修复：补 8 列 × 宽/索引 16 个只读公开属性（索引 = 列表序 + 1，列 0 是封面列）。UIA 实测：平铺 11 行 realized、8 列头矩形依次排开且宽度符合默认（488/138/158/52/48/68/48/68）；分组组头+曲目行交替渲染、组内封面缩略图正常；两种模式截图目验通过。测试教训：UIA 元素存在 ≠ 视觉可见，必须用 BoundingRectangle 判渲染。
  - ②悬浮窗"完全不能用"（用户实测的是修复前版本）根因：按下即 DragMove() 会吞掉双击第二击 → 双击回主窗永远失效。修复：阈值拖动（移动 >4px 才进入 DragMove）+ 按下事件 ClickCount>=2 直接回主窗。自动化实测：按钮④→迷你窗开（340×104 @ MiniPos）+主窗隐藏→回主窗按钮/Esc→主窗恢复 ✓；双击/拖动留给用户实机（WPF 深度依赖真实输入状态，SendInput/PostMessage 模拟均不可靠，键盘注入可用）。
  - ③迷你窗 460×320 尺寸 bug（实测发现）：WPF-UI FluentWindow + WindowStyle=None 时显式 Width/Height 被破坏（GetWindowRect 与 GetClientRect 均为 460×320，非 DPI 缩放）。修复：迷你窗换原生 Window + AllowsTransparency=True（桌面歌词窗同款已验证），340×104 恢复，Border 圆角不变。
  - harness 回归 56 项（downloads 19 + grouping 13 + dsp 2 + theme 22）零失败。
  - **列排序按钮排查（2026-08-16，用户反馈"上下按钮没反应"）**：裁剪截图确认按钮渲染正常（每行 ↑↓ 存在）；UIA 实测（加 AutomationProperties.Name 后）Invoke「歌手上移」→ config.columns 正确交换——Command 链路本身正常，按钮功能可用（用户可能受旧版或交互反馈弱影响）。UIA 之前找不到按钮是 ItemsControl 内 Button 的 AutomationPeer 暴露问题（仅影响自动化）。顺手补上移/下移按钮 AutomationProperties.Name（无障碍）。
  - **L3.1 增补（2026-08-16，用户反馈；预设已按用户意见删除，见下条）**：①**恢复默认外观**（颜色区块底部按钮）：复位到 **P4 收官原样**——行高 72/分组默认展开/**无组头封面**（P4 时代组头就是细行）/字体默认/字号 100%/强调色跟随封面/默认透明度/8 列默认顺序与默认列宽；②**风格预设下拉框**（与恢复默认按钮并排一行）：**4 套完整列表风格**（非仅换色）——经典（P4 原样）/简洁三列（行高 56+仅标题歌手时长 3 列+列宽 460/160/64+海洋蓝）/信息全览（8 列加宽+组头封面+字号 105%+青绿）/沉浸大图（行高 110+组头封面+核心 4 列+字号 110%+落日橙），每套含强调色/行高/字号/透明度/组头封面/列子集/列宽全字段；应用后设置页控件同步（ReloadUiProperties + 下拉保持选中，恢复默认后下拉回"经典"）。UIA 实测：简洁三列 → columns=[Title,Artist,Duration]/row=56/accent=#0288D1/cover=false/widths={460,160,64} ✓；沉浸大图 → 4 列/row=110/scale=1.1/cover=true ✓；恢复默认 → cover=false/8 列默认序/widths 空 ✓；两套风格截图目验（简洁紧凑/沉浸大图）✓。harness 回归 56 项零失败。
  - **风格预设删除（2026-08-16，用户反馈三轮）**：首轮 6 套按钮组、二轮 4 套下拉完整列表风格（含列子集/列宽/组头封面/字号/行高/强调色/透明度，UIA 实测与截图目验均有效），用户评估"仅凭当前自定义选项调不出第二种好看的样子，设计时只考虑了一种表现形式"——**预设整体删除**，保留恢复默认 + 自定义，留待以后版本做主题。
  - **L3.1 增补二（2026-08-16，用户四反馈）**：①「下一首播放」误为直接播放 → 改为**纯插队不打断**（PlayNextTracks 只 InsertAfterCurrent，队列空才起播；用户语义：插队=下一首播放，立即播=播放）。②「属性」SHObjectProperties 在 Win11 不弹窗 → 换**自绘曲目属性对话框**（TrackPropertiesDialog：标题/歌手/文件/路径/大小/时长/格式/采样率/位深/码率/艺术家/专辑，实测截图确认弹出）。③**歌词来源设置**：主列表右键菜单加「歌词来源 ▸」子菜单（自动/优先 .lrc/优先内嵌/仅在线，按选中曲目持久化 LyricsCacheStore + 当前播放曲目即时重载）；设置页外观区歌词组加「歌词来源」默认值下拉（Ui.LyricDefaultPreference，LyricsService.GetPreference 未单独设置时回退全局默认）。④**菜单风格切换不生效** → 弃 DynamicResource 方案，**菜单打开时显式应用**（ApplyMenuStyle 读 config 设 Style：ClassicContextMenuStyle 或 null），主列表/在线菜单接入；实测 classic/modern 两种配置下菜单均正确打开（经典白底细分隔线/现代 WPF-UI），裁剪截图对比确认。harness 233 项零失败。
  - **L3.1 增补（2026-08-16，用户要求）**：①**桌面歌词手动拉长缩短**——原有右侧调宽柄仅 10×36 且窗口外露出（Margin -6）难命中、拖动无显式捕获 → 加大 18×48、窗口内露出更多、Mouse.Capture 显式捕获（拖动跨窗口边缘不丢事件）+ AutomationProperties.Name；实测坐标拖动 320→440、config 持久化 ✓。②**主列表右键菜单（从无到有）**：播放 / 下一首播放（PlaybackList.InsertAfterCurrent 插队+立即播放，harness 5 项断言）/ 打开所在文件夹（explorer /select）/ 复制（剪贴板 FileDropList，实测「Aimer - カタオモイ.flac」入剪贴板 ✓）/ 移动到…（OpenFolderDialog+File.Move）/ 重命名（InputDialog+File.Move）/ 删除（确认+File.Delete）/ 属性（SHObjectProperties 原生属性页）/ 添加到歌单（动态子菜单，P4 已有命令）/ 移出歌单（仅歌单页，PlaylistService.RemoveTracks+重建页）；右键先选中命中行（多选保持），ContextMenuOpening 动态显隐（Tag 遍历定位，菜单独立命名作用域 FindName 不可靠）。③**复古原生右键菜单**：设置页外观区「菜单风格」开关（Ui.ClassicMenus）——经典 = BasedOn 系统键的 ContextMenu/MenuItem/Separator keyed 样式（WPF 默认 Win32 质感）+ ClassicMenuStyleSelector（ItemContainerStyle 直接设 MenuItem 样式会在 Separator 上抛类型不匹配，实测崩溃）→ 菜单全部加 Style={DynamicResource MenuStyleOverride}（主列表/在线/侧边栏歌单/大歌词/播放模式/输出设备）；ThemeService 设 MenuStyleOverride/SeparatorStyleOverride（null 回退 WPF-UI 现代）。实测：经典菜单弹出 10 项、条件项正确显隐、截图目验复古质感。harness 全模式 233 项零失败。
  - **桌面歌词两 bug 修复（2026-08-16，用户反馈）**：①**首次打开不灵敏/要点多次**——两层根因：播放条小按钮 ToolTip（悬停 400ms 弹出后拦截点击，首次找按钮时悬停久尤其明显）→ 移除设置/大歌词/桌面歌词/迷你窗 4 个按钮 ToolTip（语义走 AutomationProperties.Name，UIA 实测 Invoke 一次即开/关）；且首次创建分支缺 UpdateDesktopLyrics()（打开瞬间窗口空白，用户以为没点中反复点）→ 补调用（实测打开即有歌词文字）。②**歌词省略号**——PrimaryText/SecondaryText 的 TextTrimming=CharacterEllipsis 截断长句 → 改 TextWrapping=Wrap 完整换行（高度沿用单双行 70/120，常见歌词两行内完整）。harness 回归 56 项零失败。
  - **L3.1 设置"没作用"排查（2026-08-16 凌晨，用户截图反馈）**：UIA 实测设置页全部交互链路正常（行高 ComboBox→config.rowHeight=110、列勾选→config.columns 移除 Bitrate、滑块/开关同理），配置→渲染链路也正常（行高 110、隐藏列、组头封面关闭、字号放大均生效）。**唯一真 bug：字号缩放对列表行/组头/右侧标题不生效**——曲目行（12/10.5）、组头（13/11/10）、右侧面板标题（14）全部硬编码 FontSize，UiFontSizeBody/Small/Title 资源没被引用。修复：MainWindow.xaml 行模板 10 处 10.5、行标题 12、组头 13、年份/箭头 11/10、右侧标题 14 全改 DynamicResource（UiFontSizeBody/Small/Title），字号缩放 90–125% 现全界面生效（实测 1.25：行标题 20px、组头 20px、右侧标题 24px）。其余"没作用"解释：分组相关项（默认展开/组头封面）在平铺模式下无可见效果；设置页是独立页面，改动需回列表页查看。
...
- step5 全局热键 ✔：RegisterHotKey 预设 Ctrl+Alt+P/←/→（MOD_NOREPEAT），HwndSource 钩子收 WM_HOTKEY 分发到播放命令；默认全关（UiConfig.GlobalHotkeysEnabled），设置快捷键页开关即时生效；被占用组合不抢注册、其余照常生效，占用清单随 MessageBox 逐条提示（不静默）；触发记日志便于验证。冒烟：无冲突注册成功且 WM_HOTKEY 触发日志齐全；模拟占用 Ctrl+Alt+P 后弹窗提示且上一/下一曲照常触发（注：DSH 会话 SendInput 不注入键盘，改用 PostMessage WM_HOTKEY 等价验证，真实按键待用户目验）。
- step6 设置页补全 ✔：「歌词」页由 L1 交付（字体/字重/桌面歌词个性化，本步无新增）；「行为」页 = 关闭行为（step4）+ 启动恢复策略（恢复上次曲目信息不自动播放 / 回到上次页面，UiConfig.RestoreLastTrack/RestoreLastNav 默认开，ShellViewModel 启动时按开关执行）；「关于」页 = 版本（AssemblyInformationalVersion）+ BASS 非商业授权提示 + 开源组件清单（WPF-UI/CommunityToolkit/Serilog/TagLibSharp/ManagedBass/Sqlite）+ ChKSz/GD 接口署名；在线组 API Key 改密文（隐藏态只回显 ••••+尾4位、眼睛按钮临时明文可编辑、隐藏态只读，KeyMasked 同步尾4位）。UIA 驱动截图目验：8 个分页全部渲染、在线页掩码/明文切换、行为页三开关、关于页内容齐全。
...
- 子代理审查结论（2026-08-15，2 高 4 中全修复，harness 全量 220 项零失败）：① DownloadService 出队与 CTS 清除不同锁 → 并发下静默丢最后一个任务，改同锁原子操作；② Dispose 后 Enqueue 用已销毁 HttpClient → 入队前置守卫返回失败项；③ GdSource _subSourceOk 普通 Dictionary 跨线程 → ConcurrentDictionary；④ 搜索 Track.Name 可空 → 空串兜底；⑤ ChkszSource 码率单位 bps 直接当 kbps（如 320000 标成 320000kbps）→ >10000 时 /1000；⑥ 试听态 Next/Prev 未退出预览 → PlayCurrentOrSkip 补 ExitOnlinePreview，预览另加 CancellationTokenSource 防悬挂；⑦ GD 歌词与搜索共用一个令牌桶互相饿死 → 独立 _lyricBucket；⑧ 命名模板空段清理残留双斜杠 → 模板渲染统一消段；⑨ OnlineSources 死代码清理。全部修复已随 f60b71d 提交，发布产物已重建。
- 实机反馈修复（2026-08-16，用户目验后反馈 6 条，除第 1 条断网复测通过外全部修复并随 3fb21ca 提交，全 harness 220 项零失败+发布重建）：① 右键菜单试听/下载均无效（真鼠标）——原方案菜单弹出后在点击时用鼠标位置命中测试取行，弹出层遮挡+点击偏移导致失效，改右键按下时就捕获所在行（PreviewMouseRightButtonDown），菜单点击直接用捕获值；② 音质显示裸数字 999 不直观 → QualityFormat（999=24bit 无损/740=16bit 无损/其余 kbps），搜索行/下载完成/试听状态行/下拉选项全部换可读标注；③ 设置-在线页新增 下载位置（浏览选择，改即生效）+音质档（默认档持久化）+④ 通用 API 设置：GD 与网易云地址可自行输入（Key 仍可留空），留空回落官方默认，非法地址提示不落盘，请求时读配置即时生效无需重启；⑤ 关于页删除在线接口文字行。剩余待用户复验：右键菜单修复后真实鼠标点击。
- 右键菜单治根与实证（2026-08-15，用户复测仍失效后追查）：①根因 1：WPF 将 Style Setter 里元素的 XAML Click 事件处理器静默丢弃（已知限制）——这是最初就失效的原因；②根因 2：试图用 ContextMenuOpening 挂 AddHandler 时，OriginalSource 是行内文本元素而非 ListBoxItem，取 fe.ContextMenu 为 null 导致挂接未执行；③修复：ContextMenuOpening 中从 OriginalSource 向上找 ListBoxItem（顺便捕获行数据），用其 ContextMenu 实例 AddHandler(MenuItem.ClickEvent)，同一实例只挂一次；点击时优先用 ContextMenu.PlacementTarget（框架打开时自动设为所属行），捕获值作网底。④ 实证（真实输入注入，非 UIA Invoke）：右键→试听 → ChKSz 取流 163_music + URL 流 ASIO 192kHz 打开播放（时长 4:24 与曲目一致，截图确认 UI ：顶部信息栏/右侧面板/状态行均更新）；右键→下载 → 入队状态文本确认 + 下载完成落盘（命名模板 YuNi/Peridot/夜に駆ける (Cover).flac）+ 163_lyric 歌词 + 完成后自动增量扫描入库。随 3e8bc66 提交，发布已重建。
- 实机崩溃修复与文案精简（2026-08-15，用户复测）：① GD 下载报错闪退根因：下载管理页行模板 ProgressBar.Value 默认 TwoWay 绑定只读的 DownloadRow.Progress → XamlParseException 但只在该页渲染行时触发（测试时未导航过下载页故未现）；异常发生在布局阶段，每次弹窗点掉后布局重试再弹→弹窗风暴→资源耗尽（无法创建新的堆栈防护页面）。修复：Progress 绑定改 Mode=OneWay；另给 OnDispatcherUnhandledException 加节流（同一异常 10 秒内重复只记日志，连续 5 次直接退出）。② 下载前预检：下载目录未设置时提示不入队。③ 删除全部非必要解释文字（搜索/下载/设置各页副标题与说明段、Key与地址提示、歌词/快捷键/行为说明、关于页 BASS 授权与开源组件行）；状态行简化（找到 N 条、已加入下载队列），保留功能反馈与额度提示。实证：下载页进度行渲染无异常（截图：下载中 10% 黄色进度条）+下载完成落盘；全 harness 220 项零失败。随 aa4134b 提交。
- 实机反馈第二轮（2026-08-15）：① 设置-在线页改通用 API 列表（每条：类型 GD 兼容/网易云兼容 + 地址 + 可选 Key，可自行增删，为引入新 API 类型留基础；旧 gdApiUrl/chkszApiUrl/apiKey 加载时自动迁移，GdSource 取 Kind=gd 第一条、ChkszClient 取 Kind=chksz 带 Key 第一条）；② 下载时弹窗选择目标（媒体库根/一级子文件夹 + 自定义浏览，选完记作默认），设置页下载位置行移除；③ 下载管理增加取消（排队/下载中可取消，当前任务取消不中断队列）；顺带修两个隐藏 bug：DownloadRow 普通类不通知导致进度/状态永不刷新（实现 INotifyPropertyChanged + 全量 Refresh），且下载完成触发扫描→RebuildNavigation(libraryChanged)→重建当前页→历史任务丢失（DownloadService 保留最近 50 条任务历史）；④ 恢复关于页 BASS 授权/开源组件声明；⑤ 删其余解释文字（输出页后端提示、导航项“GD/网易云”“串行队列”提示）。实证（真实输入）：下载全链路 弹窗选目录→进度 33%→取消→“已取消”；设置页 API 列表渲染+额度正常。随 334438c 提交。
...
**XAML 铁律（本阶段强制）**：列表/歌词相关模板一律在元素上内联定义，禁止放资源字典隐式模板；歌词控件与分组列表不使用 WPF-UI 的控件样式（防主题样式覆盖模板）；每步完成后子代理只读审查模板/绑定/资源键。

### P4 在线搜索与播放

> **P4 修订 v2（2026-08-14，用户定调：本质是本地播放器，在线只要"搜索、试听、下载"三件套；本节优先于下方原文、提示词与修订 v1）**：
> 范围裁剪：在线曲目**不进**歌单/播放队列（playlist_items 的 online 结构弃用）；网易云歌单同步整项移入 Backlog；原 P5 的下载并入本阶段（P5 随之撤销）。执行顺序也已调整：先 L1、L2 两个本地阶段，P4 最后做。
> 1. **源抽象 IOnlineSource**：GD Studio API（文档 docs/GD音乐API.txt；无 Key 多音源，br 740/999=16/24bit 无损）为默认源承担搜索/取流/歌词；ChkszClient 收编为网易云兜底源。GD 打样纪律同 ChKSz（样本存 docs/api-samples/gd/，注明直链有时效）；无限流文档→保守 10 次/分令牌桶 + 失败指数退避 + 逐源可用性探测（不可用源下拉灰显）。
> 2. **搜索**：侧栏新增「在线搜索」入口（不占播放条按钮）；关键词 + 音源下拉 + 翻页；结果列 标题/歌手/专辑/时长/音质标注；支持 `[source]_album` 拉整张专辑曲目列表。
> 3. **试听**：双击结果即播——URL 流走既有 ASIO 链路、缓冲提示、实际音质标注、解析失败逐级降档。试听是**临时播放**：不写入任何歌单/队列，切走即结束；歌词照常显示（GD lyric）。
> 4. **下载**（原 P5 内容并入）：结果行与试听态一键下载；串行队列（任务间隔 ≥4s）；默认音质 999→740→320 逐级回退并标注实际；TagLibSharp 写 标题/歌手/专辑/曲号/封面，有词写同名 .lrc；按命名模板 {AlbumArtist}/{Album}/{TrackNo} - {Title}.{ext} 落入设置的下载目录；完成触发增量扫描自动入库；与库内重复（标题+歌手同且时长差<2s）先提示；失败自动重试 1 次；下载管理页显示队列/进度/结果。走 ChKSz 的操作保留额度预估提示。
> 5. **歌词链插入 GD**（本地歌零额度）：.lrc > 内嵌 > 缓存 > GD(netease，复用已匹配 id) > ChKSz。
> 验收：任一可用音源 搜到→试听出声（≤3s、ASIO 输出）→下载落库且标签/封面/.lrc 齐全；GD 故障网易云自动落回 ChKSz；试听不污染本地队列；断网时本地播放零影响（顺带补 P3 遗留的断网实机复测）。
...
> **P6 修订（2026-08-16，规划方收官版；SMTC/全局热键/托盘已在 L2 完成，此节以修订为准）**：
> 1. **单实例 + 文件关联**：Mutex 单实例；第二实例把命令行文件经命名管道转交运行实例（入队并播放）后退出。HKCU 注册（免管理员）：mp3/flac/m4a/ape/wv/ogg/opus/wav/aiff 的"打开方式"+ 默认图标；双击文件/多选打开/拖到 exe 图标三种路径都要通。
> 2. **任务栏缩略图按钮**（上一曲/播放暂停/下一曲）与任务栏进度条（可选）。
> 3. **异常兜底**：AppDomain/Dispatcher/Task 三层未处理异常→写日志+友好弹窗（附日志路径），不闪退。config.json 损坏时备份重建不崩。
> 4. **首次运行引导**：检测到无配置时的轻量引导（选曲库目录→选输出设备→可跳过的 Key 填写）。
> 5. **发布工程**：publish.ps1 一键产出 **self-contained** 便携 zip（沿用现行路线，含全部原生 dll）；exe 版本号/图标/产品元数据；面向用户的 README（快速上手 + ASIO 指引摘要 + 许可声明：BASS 非商业、GD API CC BY-NC 个人使用、开源组件清单）。
> 6. **性能收尾**：实测并记录 启动到可交互时间（目标 ≤1.5s）、万曲扫描、连续播放 2 小时内存（<300MB）、迷你窗+频谱开启时 CPU。
...
**P6 首轮审计记录（2026-08-17，规划方，对象=gpt5.6sol 的全面优化交付）**：
✅ 核验通过——提交链与备份标签规范；native/x64 精确 9 DLL（**bass_aac GPLv2 剔除正确且高价值**，AAC 走系统解码器符合 PLAN 既有后路）；THIRD-PARTY-NOTICES.md + licenses/ 齐备；publish.ps1 安全机制实存（暂存隔离/敏感熔断/chksz 流扫描/原子替换）；交付 zip 285 项、SHA-256 与报告一致、无 bass_aac 无任何用户数据；chksz_ 工作区与全历史干净。
❌ 三个缺口（P6 代码侧闭环前必补）：① **pre-commit 钩子未做**（批准清单明项）；② **ChKSz 新 Key 链路自测无证据**（报告未提及）；③ **README 停留在 P3 时代**——三级歌词链/歌词覆盖层/播放条额度指示均为已废弃描述，GD 双源、大歌词页、桌面歌词、迷你窗、快捷键等 v1.0 主体功能缺失，须整体刷新为面向用户的 v1.0 文档（快速上手/功能总览/ASIO 摘要/许可声明）。
...
- **文件关联"没有注册类"修复（2026-08-16，用户双击 flac 实机反馈）**：根因 = FileAssociation 的 ClassesRoot 与 ProgID 命令路径**丢了反斜杠**（SoftwareClasses、shellopencommand——注册落到了错误键位，真实 HKCU 未生效）。修复：ClassesRoot 补正、命令路径补 shell/open/command、补 shell/open 默认动词、.ext 加 OpenWithProgids、**先完整写 ProgID 再写 .ext 关联**、末尾 **SHChangeNotify(SHCNE_ASSOCCHANGED) 刷新关联缓存**（此前缺失——资源管理器仍走旧关联）；exe 路径加 Environment.ProcessPath 兜底。注册表结构模拟验证（写入/读取/清理 ✓），发布已更新——**待用户实机重测三路径**。
...
**P6 代码侧收官记录（2026-08-17，全面优化前历史基线）**：Release clean rebuild `0` 错误 / `0` 警告（原 21 条全部修复，无豁免）；harness 全模式 `292` 通过 / `0` 失败 / `1` 跳过（仅目标 ASIO 硬件听测）。单实例管道改为 SID+会话隔离，外部文件启动/运行期间统一 FIFO；任务栏播放状态与进度状态同步；三层异常日志改为脱敏文本并附日志路径。新增 `.githooks/pre-commit`，完整暂存区检查保留前缀，仅 PLAN 规则文本与 Harness 脱敏夹具说明可通过；故意暂存测试已拦截。真实 ChKSz 搜索链路成功（HTTP 200，额度头 Free=266 / Paid=0，Key 本体未写入报告或日志）。当时 self-contained 包 `Player-v1.0.0-win-x64.zip`：273 项、77,837,863 字节，SHA-256 `55837748159D4B21AAAFAEA5B890537B559890AAD2AD8897D80079B4C5AED365`。详见历史快照 `docs/P6-代码侧完成报告.md`。

**P6 用户环境首轮（2026-08-17）**：当前机器已实测 Shell 双击/多选/直传 exe 单实例链路、最终 zip 无 `data` 冷启动 `1295ms`、10,010 条扫描（全量 `0.6s`/增量 `0.0s`）和 60 秒内存基线（工作集 `209.0→205.4MB`）。仍需任务栏目验、频谱连续 2 小时内存和新 Windows 用户/干净电脑完整运行；完成后进入规划方终审与 v1.0。

**2026-08-17 全面优化收官（当前基线）**：已在标签 `pre-optimize-20260817-107cb8d` 与工作区外副本上保留优化前状态。音频控制面改为单飞恢复、END FIFO、曲目身份/控制意图分代、预载任务跟踪与退出排空；媒体库用不可变原子快照与无丢唤醒的 watcher 补扫队列；在线搜索/歌词/下载统一取消、latest-wins 与通知节流；主列表、窄窗三栏、定位/分组、SMTC 元数据和主题刷新均完成减负。迷你窗在本机 `DragFullWindows=0` 下改用设备像素 `SetWindowPos` 实时移动，20/20 采样帧最大滞后 `0 px`；seek 点击/滑块拖动、歌词⇄频谱、等比缩放、双击恢复/返回主窗均通过，最终模式为歌词。频谱仍严格走 `mixer DSP tap → SPSC ring → 后台 FFT`，没有新增解码/混音流 `ChannelGetData`。删除 GPLv2 `bass_aac.dll`，发布仅含 9 个 BASS DLL，并补齐第三方声明和许可。Release 重建 `0` 错误 / `0` 警告；九模式 harness `348` 通过 / `0` 失败 / `1` 跳过，媒体库端到端另有 `12 / 12` 通过；三个项目无已知 NuGet 漏洞。当前 ZIP `Player-v1.0.0-win-x64.zip`：285 项、77,744,079 字节，SHA-256 `17D539395BBDDC1AA841D54282AEB023B954FC712047E3539A6B7F1786E5A0A2`；包内无 `data`、配置、数据库、日志、保留 Key 前缀或 `bass_aac.dll`，无数据解压首启/引导/正常退出烟测通过。详见 `docs/2026-08-17-全面优化报告.md`。仍待用户环境项不变：任务栏按钮目验、2 小时频谱内存、另一台干净电脑或新 Windows 用户完整验收。
...
QQ 音乐/酷狗音源切换（ChkszClient 已预留，两家详情响应自带 `lrc` 字段可直接显示歌词）；CUE 整轨分轨支持（foobar 用户常见需求，需自写 CUE 解析 + 区间播放）；均衡器与音效（bass_fx.dll）；更多频谱/VU 形态（只能复用 mixer DSP tap 环形缓冲，禁止对解码流/混音流直接 `ChannelGetData`）；ReplayGain 回放增益；DSD 原生播放（bassdsd + DoP）；全局热键自定义；浅色主题精修；歌曲评分与智能歌单；手机遥控（局域网 HTTP，远期）。

**P4 遗留（2026-08-15 用户确认搁置）**：① GD 多端点故障转移——ApiEndpoints 里 Kind=gd 填多条地址时依次尝试（官方 API 子源不稳是实测事实，多端点=多一条命；成本低，GdSource 内部改）；② LX Music（洛雪）协议适配器——自建生态第二大家协议，待用户确有 LX 端点时再做（新 Kind + IOnlineSource 实现，需真实端点打样）。
