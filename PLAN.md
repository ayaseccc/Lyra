...
**UI-R2 专辑分组列表**：组头行（专辑名 | 艺术家 ———— 年份）；封面缩略图独占一列、随组显示；组内曲目行（碟号.曲号、标题、艺术家、时长）；当前播放行整行强调色高亮 + ▶ 标记；时长列点击可切换"剩余时间"显示；保持虚拟化，万级曲库流畅；作为可切换显示模式与平铺列表并存。

**UI-R3 封面取色主题引擎**：从当前曲目封面提取主色（缩至 ~32×32 后直方图/聚类，饱和度过低则回退默认色）；生成整套派生色（窗口背景浅 tint、面板背景、选中/高亮、强调色、文字深色）并应用到全界面（对照截图的整体染色效果）；切歌平滑过渡（300ms 级）；设置页提供三挡：跟随封面 / 固定深色 / 固定浅色。取色与派生算法抽纯函数进 harness。

**UI-R4 右侧信息栏完整版 + 星级**：大封面、星级（可点击打分，落库，主列表「等级」列同步显示与编辑）、标题、艺术家 | 专辑、作词/作曲（标签里有才显示）、歌词联动预览（复用 UI-R0 控件）；折叠状态持久化。
