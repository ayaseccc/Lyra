...
- **迷你悬浮窗**：默认显示歌词，可切换 16 柱频谱；上一首、播放暂停、下一首和进度拖动均可直接操作。窗口可自由移动并按比例缩放，双击非交互区域恢复默认尺寸，双击封面返回主窗口。右键菜单可只隐藏背景卡片（文字、封面和控件保持不透明）；「设置 → 外观」另有整窗不透明度，两者可叠加。歌词行较长时保留翻译并横向滚动原文，无翻译时才折成两行。
- 频谱样本来自 mixer DSP tap 的复制缓冲，不会额外读取或争抢 ASIO 解码/混音数据。

### 界面与个性化

- 浅色/深色底色与封面取色可以独立组合；关闭染色后使用固定主题色，迷你窗始终不参与封面染色。
- 三栏布局支持窄窗口；可调整行高、分组展开方式、列显隐/顺序/宽度、全局字体与字号、强调色、选中/悬停透明度、迷你悬浮窗不透明度。
- 设置页分为外观、歌词、播放、媒体库、在线、快捷键、系统七类。
...
默认应用内快捷键如下，除 Tab/Esc 外均可在“设置 → 快捷键”改绑：
